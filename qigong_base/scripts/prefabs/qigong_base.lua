local qigong_containers = require "mods/qigong_containers"
...
--两种音效
-- local SOUND_SWING = "dontstarve/wilson/torch_swing"
-- local SOUND_EXPLO = "dontstarve/bee/beemine_explo"
...
-- 触发进阶事件
local function onlevelup(inst, owner)
    inst.SoundEmitter:PlaySound("wilson_rework/ui/skillpoint_dropdown")
    --触发时自动打开吞噬格
...
local function judgelevelup(inst, owner)
    if inst.components.move_attack.sumattack == inst.components.move_attack.levelupattack then
        --开启进阶
        onlevelup(inst, owner)
        inst.components.move_attack:CulStepAttackPercent()
        inst.components.move_attack:ResetSumAttack()
        inst.components.move_attack.leveluping = true
    end
end
